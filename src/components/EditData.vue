...
const changesToProd = async () => {
  ...
  if (conf){
    ...
    for (const key in changes.value.patch) {
      ...
      changes.value.patch[key].start_time = DateTime.fromFormat(start_time_temp.replace(' klo', ''), "d.M.yyyy HH.mm.ss").toISO();
      changes.value.patch[key].end_time = DateTime.fromFormat(end_time_temp.replace(' klo', ''), "d.M.yyyy HH.mm.ss").toISO();
